...
// ── CHANGED: validityDays param added ────────
// ─────────────────────────────────────────────
export const approveEnrollment = async (
  enrollmentId: string,
  adminId: string,
  validityDays?: number          // undefined = lifetime access
): Promise<ServiceResponse<any>> => {
  try {
    ...
    // Calculate validUntil — null means lifetime
    const validUntil =
    ...
    // শুধু course enrollment হলে enrollmentCount বাড়াবে
if (enrollment.course) {
  await Course.updateOne({ _id: enrollment.course }, { $inc: { enrollmentCount: 1 } });
}

// tool হলে tool এর enrollmentCount বাড়াবে
if (enrollment.tool) {
    ...
    if (student && course) {
      ...
      createNotification(
        ...
        `Your enrollment in "${course.title}" has been approved!${
          validUntil
            ? ` Access valid until ${validUntil.toLocaleDateString()}.`
            : ""
        }`,
...
// ─────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────
// enrollment.service.ts এ শুধু getPendingEnrollments function টা
// এই code দিয়ে replace করো
// ─────────────────────────────────────────────────────────────────────
...
export const getPendingEnrollments = async (): Promise<ServiceResponse<any>> => {
  try {
    const enrollments = await Enrollment.find({ paymentStatus: "pending" })
      ...
      .populate("tool",    "name thumbnail price")   // ← tool populate
      .sort({ createdAt: -1 })
      .lean();
...
// ─────────────────────────────────────────────────────────────────────
// এই function টা enrollment.service.ts এর শেষে ADD করো (replace না)
// Tool এর জন্য bKash payment submit
// ─────────────────────────────────────────────────────────────────────
...
export const submitToolPayment = async ({
  ...
  toolId,
  transactionId,
  variationDays, // optional — from selected variation
}: {
  studentId: string;
  toolId: string;
  transactionId: string;
...
if (existing) return { success: false, message: "You already have access to this tool.", errors: [] }
    ...
    // If variation selected, use variation price
    ...
    // Free tool — give access immediately
    if (finalPrice <= 0) {
    ...
    // Paid — create pending enrollment
    const enrollment = await Enrollment.create({
      student: studentId,
      tool: toolId,
      itemType: "tool",
      ...
      paymentMethod: "bkash",
      transactionId: transactionId.trim(),
      // Store requested validity — admin will set it on approval
    });
...
export const getUserTools = async (userId: string): Promise<ServiceResponse<any>> => {
  try {
    const enrollments = await Enrollment.find({
      ...
      .populate("tool", "name thumbnail accessLink price")
      .sort({ createdAt: -1 })
      .lean()
 
    const now = new Date()
    const tools = enrollments.map((e: any) => {
      const isExpired =
